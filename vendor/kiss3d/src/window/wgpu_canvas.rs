...
impl WgpuCanvas {
    ...
    pub fn get_current_texture(&self) -> Option<wgpu::SurfaceTexture> {
        let surface_debug = std::env::var_os("KISS3D_SURFACE_DEBUG").is_some();
        let log_status = |msg: &str| {
            if surface_debug {
                eprintln!("[kiss3d] surface status: {msg}");
            }
        };

        let retry_after_reconfigure = || {
            let ctxt = Context::get();
            self.surface.configure(&ctxt.device, &self.surface_config);
            ...
            match self.surface.get_current_texture() {
                ...
                wgpu::CurrentSurfaceTexture::Timeout => {
                    log_status("Timeout (after reconfigure)");
                    None
                }
                wgpu::CurrentSurfaceTexture::Occluded => {
                    log_status("Occluded (after reconfigure)");
                    None
                }
                wgpu::CurrentSurfaceTexture::Outdated => {
                    log_status("Outdated (after reconfigure)");
                    None
                }
                wgpu::CurrentSurfaceTexture::Lost => {
                    log_status("Lost (after reconfigure)");
                    None
                }
                wgpu::CurrentSurfaceTexture::Validation => {
                    log_status("Validation (after reconfigure)");
                    None
                }
            }
        };
        ...
        match self.surface.get_current_texture() {
            ...
            | wgpu::CurrentSurfaceTexture::Suboptimal(texture) => Some(texture),
            wgpu::CurrentSurfaceTexture::Outdated => {
                log_status("Outdated; reconfiguring");
                retry_after_reconfigure()
            }
            wgpu::CurrentSurfaceTexture::Lost => {
                log_status("Lost; reconfiguring");
                retry_after_reconfigure()
            }
            wgpu::CurrentSurfaceTexture::Occluded => {
                // Occlusion is transient (e.g. minimized/covered window). Skip rendering
                // this frame and let the next acquire attempt recover naturally.
                log_status("Occluded");
                None
            }
            wgpu::CurrentSurfaceTexture::Timeout => {
