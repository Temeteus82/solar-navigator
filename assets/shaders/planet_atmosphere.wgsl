...
@group(#{MATERIAL_BIND_GROUP}) @binding(0) var<uniform> material: PlanetAtmosphereMaterial;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let rim = pow(clamp(1.0 - dot(normal, view_dir), 0.0, 1.0), material.params.y);
    let forward_phase = pow(max(dot(view_dir, light_dir), 0.0), material.params.z);

    let scatter = rim * (0.25 + 0.75 * n_dot_l) + forward_phase * 0.55;
    let alpha = clamp(scatter * material.params.x, 0.0, 1.0);
    let color = material.tint.rgb * scatter * material.params.w;
